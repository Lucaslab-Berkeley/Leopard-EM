...
def resolve_match_template_backend(backend: str) -> tuple[str, bool]:
    ...
    -------
    tuple[str, bool]
        ``(cc_mode, use_mip_mask)`` where ``cc_mode`` is ``streamed`` or ``batched``.
    """
    if backend in (BACKEND_STREAMED, BACKEND_BATCHED):
    ...
    raise ValueError(
        ...
        f"{BACKEND_STREAMED!r}, {BACKEND_BATCHED!r}, "
    ...
    if not use_mip_mask:
        if orientation_eligible is not None:
            raise ValueError(
                ...
                f"{BACKEND_STREAMED!r} or {BACKEND_BATCHED!r}."
            )
        return
...
def core_match_template(
    ...
    backend : str, optional
        Cross-correlation and MIP policy: ``streamed``, ``batched``,
