...
def core_inspect_template(
    ...
    frc_stack = torch.cat([torch.from_numpy(r["frc_stack"]) for r in results.values()])
    particle_indices = torch.cat(
        [torch.from_numpy(r["particle_indices"]) for r in results.values()]
    )
    sort_indices = torch.argsort(particle_indices)
    sorted_frc = frc_stack[sort_indices]
    frequency_bins = torch.from_numpy(next(iter(results.values()))["frequency_bins"])
...
def _iter_refine_particle_frc_batches(
    ...
    frc_per_batch = len(defocus_offsets) * len(pixel_size_offsets) * batch_size
    tqdm_iter = tqdm.tqdm(
        range(num_batches),
        total=num_batches,
        desc=f"{description} particle {particle_index} on device {device_id}",
        leave=False,
        position=device_id + torch.cuda.device_count(),
        unit="frc",
        unit_scale=frc_per_batch,
    )
