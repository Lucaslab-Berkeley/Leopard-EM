...
def _stack_row_for_best_particle(df: pd.DataFrame) -> int:
    ...
    raise ValueError(
        f"No known score column found; expected one of {SCORE_COLUMNS!r}."
    )
...
def main() -> None:
    ...
    # Pool CC over (pixel, defocus, map) per orientation, show top K for that row.
